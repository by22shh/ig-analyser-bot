...
import {
  ...
  MockInstagramProfileProvider
} from "./instagram/apify.adapter.js";
...
import {
  MockFaceCheckAdapter,
  RealFaceCheckAdapter
} from "./photo-search/adapters/facecheck.adapter.js";
...
export function createServices() {
  ...
  const users = new UserService(prisma, storage);
  const wizard = new WizardStateService(prisma);
  const instagram =
    providerMode.apify === "real"
      ? new ApifyInstagramProfileProvider()
      : new MockInstagramProfileProvider();
  const llm =
    providerMode.openrouter === "real" ? new OpenRouterLlmProvider() : new MockLlmProvider();
  const pdf = env.APP_ENV === "test" ? new MockPdfAdapter() : new PlaywrightPdfAdapter();
  const reports = new ReportService(prisma, storage, pdf);
  const yookassa =
    providerMode.yookassa === "real" ? new RealYooKassaAdapter() : new MockYooKassaAdapter();
  const payments = new PaymentService(prisma, credits, yookassa);
  const facecheck =
    providerMode.facecheck === "real" ? new RealFaceCheckAdapter() : new MockFaceCheckAdapter();
  const photoSearch = new PhotoSearchService(prisma, credits);
  const analysis = new AnalysisService(prisma, credits);
  const chat = new ReportChatService(prisma, llm, credits);
