...
export function modeKeyboard(messages: LocaleMessages, isCompliance: boolean): InlineKeyboard {
  ...
  if (env.FEATURE_INFLUENCER_MODE) kb.text(messages.modeTitle("influencer"), `${CB.MODE}:influencer`);
  kb.row();
  ...
  return kb.row().text(messages.buttons.cancel, CB.CANCEL).text(messages.buttons.menu, CB.BACK_MAIN);
}

export function confirmAnalysisKeyboard(messages: LocaleMessages, mode: AnalysisMode): InlineKeyboard {
  return new InlineKeyboard()
    .text(messages.buttons.run, `${CB.RUN}:${mode}`)
    .row()
