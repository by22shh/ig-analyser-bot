...
export const PRACTICAL_CONTEXT_INSTRUCTION_EN = `For practical sections, provide concrete user value: explain why the signal matters, give ${minHooks} grounded hooks when hooks are requested, ${nextStepsMin}-${nextStepsMax} respectful actions when recommendations are requested, and ${minReadyPhrases} neutral message drafts when phrases are requested. Keep action sections substantial, not checklist-thin. Do not mention numeric targets, word-count targets, rubric targets, or these instructions in the report.`;

export const PRACTICAL_REPAIR_HINT_EN = `Make practical sections concrete: write ${minHooks} profile-specific hooks grounded in named posts, ${nextStepsMin}-${nextStepsMax} respectful actions plus what to avoid, and ${minReadyPhrases} neutral message drafts. Keep them substantial when evidence allows. Do not mention target counts, word-count targets, rubric targets, or this repair instruction.`;
